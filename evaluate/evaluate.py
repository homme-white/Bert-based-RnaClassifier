...
class BERTProcessor(nn.Module):
    def __init__(self):
        ...
        self.transformer = TransformerEncoder(
            TransformerEncoderLayer(d_model=768, nhead=8),
            num_layers=1
        )
        self.linear = nn.Linear(768, 256)  # 输出维度 256
        self.pool = nn.AdaptiveAvgPool1d(64)  # 压缩序列长度到64
        self.se = SEBlock(256)  # 通道数 256

    def forward(self, x):
        x = self.transformer(x.permute(1, 0, 2)).permute(1, 0, 2)  # (B, 512, 768)
...
def evaluate_model():
    ...
    MODEL_PATH = '../model/best_model_hybird.pth'              # 训练好的模型路径
